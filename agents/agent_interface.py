...
from agents.s5_actor_critic import (
    S5ActorCritic, StackedEncoderModel, 
    init_S5SSM, make_DPLR_HiPPO, 
    S5ActorWithDoubleCritic,
    S5ActorWithConditionalCritic
)
...
class MLPActorCriticPolicy(AgentPolicy):
    ...
    def __init__(self, action_dim, obs_dim, activation="tanh", fc_hidden_dim=64):
        ...
        Args:
            ...
            fc_hidden_dim: int, hidden layer dimension for the MLP
        """
...
class S5ActorWithDoubleCriticPolicy(S5ActorCriticPolicy):
    """Policy wrapper for S5 Actor-Critic with double critics"""
    def __init__(self, action_dim, obs_dim, 
                 d_model=16, ssm_size=16, 
                 ssm_n_layers=2, blocks=1,
                 fc_hidden_dim=64,
        ...
        # Initialize the network instance once
        self.network = S5ActorWithDoubleCritic(
            action_dim,
            ssm_init_fn=self.ssm_init_fn,
            fc_hidden_dim=self.fc_hidden_dim,
            fc_n_layers=self.fc_n_layers,
    ...
    def get_action_value_policy(self, params, obs, done, avail_actions, hstate, rng, 
        ...
        Shape of hstate should correspond to (1, batch_size, -1)
        """
        new_hstate, pi, (val1, val2) = self.network.apply(params, hstate, (obs, done, avail_actions))
        action = pi.sample(seed=rng)
        return action, (val1, val2), pi, new_hstate


class S5ActorWithConditionalCriticPolicy(S5ActorCriticPolicy):
    """Policy wrapper for S5 Actor With Conditional Critic"""
    ...
    def __init__(self, action_dim, obs_dim, pop_size,
                 ...
                 s5_no_reset=False):
        """
        Args are all the same as S5ActorCriticPolicy, except for pop_size
        """
        AgentPolicy.__init__(self, action_dim, obs_dim)
        self.pop_size = pop_size # ADDED ARG
        self.d_model = d_model
        self.ssm_size = ssm_size
        self.ssm_n_layers = ssm_n_layers
        self.blocks = blocks
        ...
        # Initialize the network instance once
        self.network = S5ActorWithConditionalCritic(
            action_dim,
            ssm_init_fn=self.ssm_init_fn,
            fc_hidden_dim=self.fc_hidden_dim,
            fc_n_layers=self.fc_n_layers,
    ...
    def get_action(self, params, obs, done, avail_actions, hstate, rng, 
                   aux_obs=None, env_state=None, test_mode=False):
        """Get actions. Since only the critic uses the agent id, we pass in a dummy vector to represent the one-hot agent id."""
        dummy_agent_id = jnp.zeros(obs.shape[:-1] + (self.pop_size,))
        new_hstate, pi, _ = self.network.apply(params, hstate, (obs, dummy_agent_id, done, avail_actions))
        action = jax.lax.cond(test_mode, 
                              lambda: pi.mode(), 
                              lambda: pi.sample(seed=rng))
        return action, new_hstate
    ...
    def get_action_value_policy(self, params, obs, done, avail_actions, hstate, rng, 
                                aux_obs=None, env_state=None):
        """Get actions, values, and policy for the S5 Actor With Conditional Critic policy.
        The auxiliary observation should be used to pass in the agent ids that we wish to predict 
        values for.
        """
        new_hstate, pi, val = self.network.apply(params, hstate, (obs, aux_obs, done, avail_actions))
        action = pi.sample(seed=rng)
        return action, val, pi, new_hstate

    def init_params(self, rng):
        """Initialize parameters for the S5 Actor With Conditional Critic policy."""
        batch_size = 1
        # Initialize hidden state
        init_hstate = self.init_hstate(batch_size)
        ...
        # Create dummy inputs
        dummy_obs = jnp.zeros((1, batch_size, self.obs_dim))
        dummy_ids = jnp.zeros((1, batch_size, self.pop_size,))
        dummy_done = jnp.zeros((1, batch_size))
        dummy_avail = jnp.ones((1, batch_size, self.action_dim))
        dummy_x = (dummy_obs, dummy_ids, dummy_done, dummy_avail)

        # Initialize model using the pre-initialized network
        return self.network.init(rng, init_hstate, dummy_x)


class PseudoS5ActorWithConditionalCriticPolicy(S5ActorWithConditionalCriticPolicy):
    """Enables S5ActorWithConditionalCriticPolicy to act as an MLPActorCriticPolicy.
    by passing in a dummy agent id.
    """
    def get_action_value_policy(self, params, obs, done, avail_actions, hstate, rng, 
                                aux_obs=None, env_state=None):
